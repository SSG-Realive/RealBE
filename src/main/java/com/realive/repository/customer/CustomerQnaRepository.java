...
public interface CustomerQnaRepository extends JpaRepository<CustomerQna, Long> {
    ...
    // 고객ID로 내 문의 리스트 가져오기
    List<SellerQna> findByCustomerIdOrderByIdDesc(Long customerId);

    // QnA ID, 고객ID로 내 문의 상세 가져오기
    @Query("SELECT q FROM CustomerQna q WHERE q.id = :id AND q.customer.id = :customerId")
    Optional<SellerQna> findByIdAndCustomerId(@Param("id") Long id, @Param("customerId") Long customerId);

    // 상품ID로 상품 문의 리스트 가져오기
    List<SellerQna> findByProductIdOrderByIdDesc(Long productId);

    // [추가] 모든 고객 Q&A를 페이징하여 가져오기 (관리자용)
    Page<SellerQna> findAll(Pageable pageable);

    // [추가] 답변되지 않은 고객 Q&A를 페이징하여 가져오기 (관리자용)
    Page<SellerQna> findByIsAnsweredFalse(Pageable pageable);

    // 판매자ID로 고객 문의 리스트 가져오기
